...
class DyConv3Group(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, bias=False, reduction=16):
        ...
        self.attention = nn.Sequential(
            ...
            nn.Conv2d(reduction_channels, reduction_channels, kernel_size=3, groups=reduction_channels, bias=False),
...
def test():
    ...
    conv = DyConv3Group(x.size(1), 64, 3, padding=1)
    y = conv(x)
    print(y.size())
