...
__all__ = ['DyCBAMConv', 'DyCBAMConv_2', 'DyCBAMConv_3', 'DyCBAMConv_4', 'DyCBAMConv_5']
...
class DyCBAMConv_5(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False):
        super(DyCBAMConv_5, self).__init__()
    ...
    def forward(self, x):
        ...
        weights_avg = self.attention(gap)
        weights_max = self.attention(mp)
        weights = self.sigmoid(weights_avg + weights_max).unsqueeze(dim=-1).unsqueeze(dim=-1).unsqueeze(dim=-1)
        out = weights * all_out
        out = out.sum(dim=1, keepdim=False)
