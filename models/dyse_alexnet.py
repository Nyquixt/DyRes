import torch
import torch.nn as nn

from .dyse_conv import *

__all__ = ['DySE_AlexNet']

class DySE_AlexNet(nn.Module):

    def __init__(self, num_classes=10, input_size=32):
        super(DySE_AlexNet, self).__init__()
        self.features = nn.Sequential(
            DySEConv(3, 64, kernel_size=3, stride=2, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
            DySEConv(64, 192, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
            DySEConv(192, 384, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            DySEConv(384, 256, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            DySEConv(256, 256, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
        )
        self.classifier = nn.Sequential(
            nn.Dropout(),
            nn.Linear(256 * 2 * 2 if input_size==32 else 256 * 4 * 4, 4096),
            nn.ReLU(inplace=True),
            nn.Dropout(),
            nn.Linear(4096, 1024 if num_classes == 10 else 4096),
            nn.ReLU(inplace=True),
            nn.Linear(1024, num_classes),
        )

    def forward(self, x):
        x = self.features(x)
        x = torch.flatten(x, 1)
        x = self.classifier(x)
        return x