...
__all__ = ['WeightNet', 'WeightNet_DW']
...
class WeightNet(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, reduction_ratio=16, M=2, G=2):
        ...
        self.reduce = nn.Conv2d(in_channels, input_gap, 1, 1, 0, bias=False)
...
class WeightNet_DW(nn.Module):
    def __init__(self, in_channels, kernel_size, stride=1, reduction_ratio=16, M=2, G=2):
        super().__init__()
        ...
        self.fc1 = nn.Conv2d(input_gap, self.M // self.G * in_channels, 1, 1, 0, groups=1, bias=False)
        self.sigmoid = nn.Sigmoid()
        self.fc2 = nn.Conv2d(self.M // self.G * in_channels, in_channels * kernel_size * kernel_size, 1, 1, 0, groups=in_channels)
    ...
    def forward(self, x):
        ...
        x = F.conv2d(x, weight=x_w, stride=self.stride, padding=self.padding, groups=b * self.in_channels)
        x = x.view(-1, self.in_channels, x.size(2), x.size(3))
        return x
