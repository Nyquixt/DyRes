...
__all__ = ['MS1Conv', 'MS2Conv', 'MS3Conv']

class MS1Conv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, bias=False):
        super(MS1Conv, self).__init__()
...
class MS2Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        scale_3 = self.scale_3(x)
        scale_5 = self.scale_5(x)
        scale = F.relu(scale_1 * scale_3 * scale_5)
...
def test():
    ...
    conv1 = MS1Conv(x.size(1), 64, 3, padding=1)
    y1 = conv1(x)
    print(y1.size())
    conv2 = MS1Conv(x.size(1), 64, 3, padding=1)
    y2 = conv2(x)
    print(y2.size())
