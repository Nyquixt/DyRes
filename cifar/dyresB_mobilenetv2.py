...
class DyRes_Block(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, expansion, stride, num_experts=3):
        ...
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = DyResConv(planes, planes, kernel_size=3, stride=stride, padding=1, groups=planes, num_experts=num_experts, mode='B')
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = nn.Conv2d(planes, out_planes, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn3 = nn.BatchNorm2d(out_planes)
...
class DyResB_MobileNetV2(nn.Module):
    ...
    def _make_layers(self, in_planes, num_experts):
        ...
        for expansion, out_planes, num_blocks, stride in self.cfg[:4]:
            strides = [stride] + [1]*(num_blocks-1)
            for stride in strides:
                layers.append(DyRes_Block(in_planes, out_planes, expansion, stride, num_experts))
                in_planes = out_planes

        for expansion, out_planes, num_blocks, stride in self.cfg[4:]:
            strides = [stride] + [1]*(num_blocks-1)
            for stride in strides:
                layers.append(CC_Block(in_planes, out_planes, expansion, stride, num_experts))
                in_planes = out_planes
