...
class DNL(nn.Module):
    def __init__(self, in_channels, channels, with_unary=True, whiten_type=['channel'], downsample=False, temperature=1.0):
        ...
        if 'bn_affine' in whiten_type:
            self.key_bn_affine = nn.BatchNorm1d(channels)
            self.query_bn_affine = nn.BatchNorm1d(channels)
        if 'bn' in whiten_type:
            self.key_bn = nn.BatchNorm1d(channels, affine=False)
            self.query_bn = nn.BatchNorm1d(channels, affine=False)
